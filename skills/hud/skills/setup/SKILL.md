...
**플러그인은 `statusLine` 을 등록할 수 없다.** 그래서 켜는 동작은 **settings.json 편집 한 번**이 반드시 필요하고, 이 스킬이 그걸 한다.

이유의 정본은 `../../README.md` 의 `왜 setup 을 따로 실행해야 하나` 절이다. 컴포넌트 목록 같은 사실을 여기 복사해두면 Claude Code 가 그 목록을 바꿀 때 두 곳이 갈린다.
...
> **사본 모드는 아직 한 번도 실행된 적이 없다.** 마켓플레이스 배포(`marketplace.json`)가 없어 도달 불가능한 경로다.
> 첫 마켓플레이스 설치 때 이 절과 제거 절차 4번을 실제로 검증한다. 그때까지는 미검증으로 취급한다.
