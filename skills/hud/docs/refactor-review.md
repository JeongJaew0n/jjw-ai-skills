...
## [낮음] H-5. 픽스처의 절대 epoch 가 노후한다 — **재검토 후 해소**

> **하지 않기로 결정** — 2026-09-01. 고치는 방법이 제품 원칙과 상충한다.
>
> 상대 시각으로 만들려면 `jq` 로 픽스처를 생성해야 하고, 그러면 **정적 픽스처가 실행물이 된다.** "의존성 0" 을 내세운 제품에 픽스처 생성 의존성을 들이는 셈이다. 얻는 것은 문서 예시 출력의 일치뿐이고, `README.md` 가 이미 이 동작을 설명하고 있어 함정도 아니다.

> **재검토 후 해소** — 2026-09-08. **실제로 발현해 작업을 방해했다.**
>
> 0.3.0(7일 창 리셋을 절대 시각으로 표기)이 적용됐는지 확인하려 했는데, 픽스처의 `resets_at` 이 이미 과거여서 **구버전과 0.3.0 이 똑같은 출력을 냈다.** 양쪽 모두 리셋 괄호를 생략한 탓이다. 임시로 `jq` 원라이너를 만들어 미래 시각을 넣고서야 차이가 보였다. 같은 원라이너를 이 세션에서 두 번 즉흥적으로 만들어 썼다.
>
> **위 "하지 않기로" 판단의 전제는 지금도 맞다** — 픽스처를 동적 생성으로 바꾸는 것은 여전히 틀린 방향이다. 틀린 것은 "그러니 아무것도 안 한다" 는 결론이었다.
>
> 진짜 문제는 픽스처가 **두 역할을 겸하는 것**이었다.
>
> | 역할 | 필요한 값 |
> |---|---|
> | stdin 계약 문서 | 절대 epoch — Claude Code 가 실제로 보내는 형태 |
> | 렌더 회귀 입력 | 항상 미래인 값 — 아니면 리셋 분기를 못 지남 |
>
> 그래서 픽스처는 계약 문서로 고정하고, **검증 절차에 갱신 원라이너를 추가**했다 (`README.md` 의 `직접 실행`, `setup/SKILL.md` 의 `검증`). `jq` 는 이미 모든 검증 단계가 쓰고 있어 새 의존성이 아니고, 제품 파일은 손대지 않았다.
>
> 교훈 — 노후하는 입력을 쓰는 **검증 절차**가 문제였고 픽스처 자체가 문제가 아니었다. 심각도를 낮음으로 둔 것은 맞았지만 "미실행" 결론은 틀렸다.
...
## [낮음] H-6. 스킬 이름 `setup` 이 일반적이다 — **하지 않음**

> **하지 않기로 결정** — 2026-09-01. 이름을 바꾸면 사용자 호출 문자열 `/hud:setup` 이 깨진다. 네임스페이스가 실제 충돌을 이미 막고 있다.
