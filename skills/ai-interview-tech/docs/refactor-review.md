...
> **해소** — 2026-09-01. 템플릿에 `> 기준 커밋: `<SHA>` / 작성일: <YYYY-MM-DD>` 를 넣고, 빠뜨리지 말아야 하는 이유를 본문에 적었다.
>
> **심각도가 검토 이후 올라간 항목이다.** 낮음으로 매긴 시점에는 I-3/R-4 연결이 없었다. 이제 `review-code-intent` 가 이 스펙을 의도 항목의 출처 등급 `명시` 로 채택하므로, 기준 시점이 없으면 옛 코드베이스 기준 결정을 현재 코드에 그대로 채점하게 된다. 연결을 만든 작업이 이 항목의 값어치를 올렸다.
...
> **해소 (축약형 채택)** — 2026-09-01. 두 번째 예시를 추가하지 않고 한 줄만 넣었다.
> `"예시는 Kotlin/Spring 이지만 형식만 빌리면 된다 — 핵심은 선택지를 그 코드베이스의 실제 코드로 보여주는 것이다."`
>
> 다른 언어 예시를 넣으면 매 호출마다 읽히는 파일에 20줄가량이 붙는다. 예시의 목적(코드로 선택지를 보여주는 형식 전달)은 이미 달성되므로 오해 위험만 막았다.
