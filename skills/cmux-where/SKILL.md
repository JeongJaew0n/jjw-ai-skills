...
name: cmux-where
description: |
  cmux 터미널 안에서 도는 세션이 자기 위치(창·workspace·pane·surface 와 좌/우/상/하)를
  파악하고, 같은 창의 다른 터미널을 위치로 지목해 조작할 수 있게 한다. cmux 의 계층
  모델과 자주 틀리는 함정(caller vs focused, index 와 화면 순서 불일치)을 함께 알려준다.
  사용자가 `/cmux-where` 또는 `$cmux-where` 를 명시적으로 호출했을 때만 사용한다.
  cmux·터미널·창 위치 관련 요청이 관련 분야에 해당한다는 이유만으로 자동 사용하지 않는다.
user-invocable: true
...
사용자가 `/cmux-where` 또는 `$cmux-where` 로 호출한 경우에만 이 스킬을 실행한다. 이름만 언급한 경우는 호출로 보지 않는다 — `disable-model-invocation` 이 그 경로를 차단하므로, 실행하지 말고 슬래시 호출을 안내한다. cmux 나 터미널 위치와 내용이 유사하다는 이유만으로 자동 선택하지 않는다.
