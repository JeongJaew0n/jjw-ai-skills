...
## [낮음] P-8. 제목의 슬래시 접두가 형제 스킬과 다르다 — **지적 철회**

> **철회** — 2026-09-01. 고치지 않는다. 지적 자체가 무효가 됐다.
>
> `disable-model-invocation: true` 를 적용한 뒤로 **슬래시가 유일한 호출 경로**다. 제목의 `/` 접두는 이제 부정확한 게 아니라 정확하다. 그리고 `hud:setup` 의 제목도 `# hud:setup` — 호출 문자열이다. 4개 중 2개가 이미 호출형 제목이므로 일관성 논거 자체가 성립하지 않는다.
>
> 조용히 지우지 않고 남긴다. 다음 리뷰어가 같은 지적을 반복하지 않도록.
